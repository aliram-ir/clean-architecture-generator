...
import { autoInstallNugets } from '../infrastructure/nugetInstaller';
...
|--------------------------------------------------------------------------
| ✅ Canonical Pure Registration
| ❌ No ExtensionContext
| ✅ Uses Single Source of Truth for CommandId
*/

export function registerCreateSolutionCommand(): vscode.Disposable {

    return vscode.commands.registerCommand(
        CommandIds.CreateSolution,
...
| Command Handler
|--------------------------------------------------------------------------
| Create full Clean Architecture solution
| ✅ Idempotent
| ✅ Deterministic references
*/

async function createSolutionCommand() {
    ...
    await vscode.window.withProgress(
        ...
        async (progress) => {
            ...
            try {
                ...
                // Application
                ensure(path.join(solutionRoot, `${solutionName}.Application`, 'Interfaces', 'Repositories'));
                ...
                const ref = async (from: string, to: string) => {
                    await run(`dotnet add "${from}" reference "${to}"`, solutionRoot);
                };
